...
void AMPCharacter::BeginPlay()
{
	...
	UCharacterData* MyData = NewObject<UCharacterData>();
	if (MyData)
	{
		UE_LOG(LogTemp, Warning, TEXT(
			"Name: %s | Health: %d/%d | Mana: %d | Attack: %d | Speed: %d | Defense: %d"),
			*MyData->CharacterName,	MyData->CurrentHealth,	MyData->MaxHealth,	MyData->Mana,
			MyData->Attack,	MyData->Speed,	MyData->Defense
		);
	}
	else
	{
		UE_LOG(LogTemp, Error, TEXT("UCharacterData 객체 생성 실패!"));
	}
}
